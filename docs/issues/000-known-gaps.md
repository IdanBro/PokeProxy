...
| M5 | Medium | `/stats` is unauthenticated and returns internal downstream URLs (`config/rules.json`'s `url` fields) to anyone who can reach the service. | **Part 4** — superseded by Prometheus `/metrics` on a port the public Service doesn't expose; fixing `/stats`'s auth now means fixing it twice |
| L4 | Low | `StatsRegistry.get()` keys `EndpointStats` by downstream URL via `setdefault` — bounded today by the rules file, but an unbounded-cardinality pattern if it were ever carried into a label set. | **Part 4** — must not be carried into Prometheus labels as-is; the fix is choosing better label dimensions when instrumenting, not patching `stats.py` twice |

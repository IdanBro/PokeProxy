...
scripts/
  ...
  run-e2e-now.sh                      # new — ad-hoc E2E Job at the currently-deployed tag (needs jq)
...
| # | Item | Status |
|---|---|---|
| 1 | One command takes a clean supported machine to a fully running, monitored stack | **Done** — step 8, cold `make up` from no cluster + no sealing key: exit 0, `real 10m22.139s`, all workloads healthy, ingress `401` (WORKLOG "Part 5 § Step 8") |

...
### Why Tilt earns its place, concretely

D1's one-liner ("declared dependency DAG, per-workload readiness/logs/status, live-reload and buttons") is the summary; this is what each of those actually buys, with real evidence rather than restated marketing:

| Pro | What it replaces | Evidence |
|---|---|---|
| **Fail-fast dependency ordering** | `deploy.sh`'s implicit ordering — each step assumes the previous one finished, with no declared dependency between them | The Tiltfile states `resource_deps` explicitly (`pokeproxy-helm` depends on `namespace` + `monitoring`; `ingress-probe` depends on `pokeproxy-helm`). Get the order wrong and Tilt reports it as a graph error at load time, before touching the cluster — a linear script instead fails midway with whatever error the wrong-order step happens to throw |
| **Per-resource status instead of one interleaved log** | `deploy.sh` prints one scrolling stream; distinguishing "the app is up" from "monitoring is up" means reading timestamps | A real `tilt ci` run today reported a clean per-step breakdown — image build 58s, push 11s, mock-downstream build 18s, e2e image build 8s, deploy 139s, 245.68s total — plus independent rollout tracking per workload (`pokeproxy` ready in 1m4s, `redis` in 44s), each attributable to a named resource, not a byte offset in a log file |
| **The deploy gate stays real, not templated** | A hand-rolled `helm template \| kubectl apply` would drop the release object, hooks, and `--atomic` entirely | D2: `ext://helm_resource` runs an actual `helm upgrade --install --atomic`, so the post-install E2E hook Job still gates the release and a failing hook still triggers a real Helm rollback — confirmed today: the hook's own log showed a real signed request forwarded end-to-end (`outcome: forwarded`, `status: 200`) before `tilt ci` reported success |
| **Idempotent re-run comes for free** | A bash script re-running `deploy.sh` needs its own hand-written "has this already been done" checks for every step, or it redoes everything every time | `helm_resource` wraps `helm upgrade --install`, which Helm itself diffs against the live release — re-running `make up` against an unchanged tree touches nothing (same cluster, same pod ages, next Helm revision only if something actually changed). Part 5 explicitly requires idempotent-and-safe-to-re-run; this is Helm's own machinery doing that work, not code this project had to write and maintain |
| **One graph serves both the CI path and the human path** | Otherwise a "deploy script" and a "dev helper script" duplicate the same steps and drift out of sync | `tilt ci` (used by `make up`) and `tilt up` (used by `make dev`) load the identical Tiltfile — the only difference is watch-mode vs. one-shot. There is exactly one definition of "how this stack comes up," not two |
| **On-demand actions become one click, not a memorized incantation** | Running Part 3's rollback demo, or sending signed load, meant typing out the right `curl`/`kubectl exec` command from memory or a README each time | Four `cmd_button`s (`send-signed-traffic`, `flush-redis-cache`, `break-rules`, `restore-rules`) turn those into a labeled button in the UI, discoverable without reading a script first — `break-rules`/`restore-rules` specifically exist to make Part 3's rollback scenario B repeatable on demand rather than a one-off manual edit |
| **Live-reload shortens the code-to-pod loop** | The full cycle otherwise: edit → rebuild image → repush → helm upgrade → wait for rollout | `docker_build(..., only=['pyproject.toml', 'uv.lock', 'src'])` scopes what Tilt watches, so an edit under `src/` triggers a fast rebuild-and-sync instead of a full `make up` cycle — the sub-second version of the same loop `tilt up`'s interactive mode is built around |

The cost side is real too and is what keeps this dev-only (D9): an unpinned extension fetch on a cold machine (A-13, still open), and one more tool on the prerequisites list. Weighed against a linear bash script that would have to hand-build all seven rows above itself, or not have them at all, the trade was worth it for the debugging surface this project otherwise has none of.
...
- `scripts/install-tools.sh` (bonus convenience, never part of this Part's core scope table above) was also deleted in the same pass, then restored after the user caught the doc/reality mismatch — see `docs/planning/AI_WORKFLOW.md`'s session 20 entry for the full account. It's back, along with its `Makefile install-tools` target and a `README.md` command-table row it never had before.

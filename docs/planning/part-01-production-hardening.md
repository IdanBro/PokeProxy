...
**Update — H4+H5 fixed (see WORKLOG). L3 reviewed and deliberately deferred, not fixed.** Error responses carry no `request_id` in the body (only `main.py`'s `internal_error` handler does), inconsistent and worth closing — but it's the one Low-severity item in the SHOULD FIX set, correlation is still possible via the `X-Request-ID` response header today, and the fix was fully scoped during review (inject `request.state.request_id` at `proxy.py`'s `_outcome_response()` call sites plus two `_forward_request` except blocks) rather than implemented, so time went to M2/M4/M7-CWD instead. This is a prioritization call, not an oversight — full reasoning in `WORKLOG.md` under "L3 — deliberately deferred, not missed."

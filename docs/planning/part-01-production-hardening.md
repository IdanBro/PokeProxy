...
**Deferrals re-confirmed, unchanged, all now indexed in `docs/issues/000-known-gaps.md`:** M2 (ingress-layer defense-in-depth in Part 2 plus a scoped app-level streaming fix), L3 (`request_id` in error bodies), H6 (config assumes localhost — a Part 2 topology decision), M3 (replay protection — a protocol change), M5 and L4 (Part 4), L6 (Part 2), L5 (Part 3), L1, L2, M6.

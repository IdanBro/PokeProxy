...
**Current phase:** **Part 3 — CI/CD & GitOps: steps 1–4a done and verified live; 4b implemented, live sync verification pending a push, 2026-08-23.** Plan: `docs/planning/part-03-cicd-gitops.md` (design `db4abba`). Branch `feature/gitops-ci-cd`, draft [PR #3](https://github.com/IdanBro/PokeProxy/pull/3) open against `main` as the vehicle for the rest of Part 3. Step 4a (the `deploy/envs/` move plus the N7 fix) is done. Step 4b (prod stand-in cluster + Argo CD) is implemented and partly verified; Argo CD reads from GitHub rather than the working tree, so its sync cannot be exercised until these commits are pushed.

**Step 4b detail — prod stand-in cluster and Argo CD. Implemented; live sync verification is pending a push, for a structural reason: Argo CD reads from GitHub, not the working tree, so nothing here can converge until these commits exist on a pushed branch.**
...
**Explicitly NOT yet verified:** Argo reaching Synced/Healthy · pods pulling GHCR by digest · the PostSync E2E Job under Argo · `selfHeal` reverting a hand-edit · 401 through the prod ingress on 8081 · the `build-e2e` and prod `chart-lint` CI jobs · `bootstrap-prod.sh`'s fail-loudly path (the one background run was killed mid-loop, so the wrapper's exit 0 is an artifact of the pipeline, not a pass). `deploy/envs/prod/values.yaml` currently points `e2e.image.tag` at `29d7317`, which is a **dangling reference** until CI's `build-e2e` runs; the tag/digest triples get re-seeded from that run, which is the plan's seeded-once-then-owned-by-CI handoff.

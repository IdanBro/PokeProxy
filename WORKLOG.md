...
| F-12…F-18 | Nice to have | Stale plan text (now fixed in place) · no `docs/issues/021+` for S4/N7 · promote doesn't assert its digests are pullable · `already_sealed()` never confirms the ciphertext decrypts under the reused key · no rebase-conflict handling in promote · fork PRs can't run `build-*` · the branch-protection/PAT episode has no issue write-up | see plan |

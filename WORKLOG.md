...
**Current phase:** **Part 3 — CI/CD & GitOps: steps 1–4 done and verified live, 2026-08-23.** Plan: `docs/planning/part-03-cicd-gitops.md` (design `db4abba`). Branch `feature/gitops-ci-cd`, draft [PR #3](https://github.com/IdanBro/PokeProxy/pull/3) open against `main` as the vehicle for the rest of Part 3. Step 4 is complete: 4a (the `deploy/envs/` move plus the N7 fix) and 4b (prod stand-in cluster on 8081 + Argo CD 10.4.0), both verified live. Step 5 (CI promote job, measuring commit to serving) is next.

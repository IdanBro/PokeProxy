...
**Current phase:** **Part 3 — CI/CD & GitOps: steps 1–4 done and verified live; step 5 implemented, live verification pending the PR #3 merge, 2026-08-23.** Plan: `docs/planning/part-03-cicd-gitops.md` (design `db4abba`). Branch `feature/gitops-ci-cd`, draft [PR #3](https://github.com/IdanBro/PokeProxy/pull/3) open against `main` as the vehicle for the rest of Part 3. Step 4 is complete: 4a (the `deploy/envs/` move plus the N7 fix) and 4b (prod stand-in cluster on 8081 + Argo CD 10.4.0), both verified live. Step 5 (CI promote job) is implemented; live verification and the commit-to-serving measurement follow the PR #3 merge to main.
...
| N7 | Nice to have | `scripts/seal-hmac.sh:96` rewrites `values-local.yaml` wholesale with `cat >`, discarding anything else in the file | Merge, or narrow the write to the `hmac:` key — via yq | **Fixed** — Part 3 step 4a: `write_encrypted_value()` does a targeted `sed` replacement plus a post-write assertion. Deviated from the planned yq (unavailable from apt on Ubuntu 22.04); CI uses yq at step 5 |

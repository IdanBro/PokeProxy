...
**Four SHOULD FIX still open:** S1 `components.{pokeproxy,redis}.enabled` declared but never read (same bug class as the mock-downstream fix in `721b8fc` — renders a Deployment whose ServiceAccount is never created; pod creation is then `Forbidden`). S2 `enableServiceLinks` left default, so Kubernetes injects `POKEPROXY_PORT=tcp://10.43.93.39:8000` into every pod — pokeproxy is saved only by env precedence over its ConfigMap. S3 no runnable deploy procedure or top-level README anywhere in the tree (deliverable 8; Part 5 owns the one-command version). S4 `values-prod.yaml` renders rules pointing at the Service it disables — already documented, but a `fail` would turn a runtime 502 into a render-time error. **Six NICE TO HAVE** (N1–N6) are listed in the planning doc's audit section.
...
**Next:**
0. **B1 and B2 are fixed** (2026-08-23) — see above and `docs/issues/017-sealed-secret-key-portability.md`, `docs/issues/018-namespace-not-tracked.md`. S1–S4 are chart hygiene that can ride along with Part 3 or wait.
...
| S1 | Should fix | `components.{pokeproxy,redis}.enabled` declared but never read — renders a Deployment whose ServiceAccount is never created | Gate the templates, or delete the unused keys | Open |
| S2 | Should fix | `enableServiceLinks` default-true injects `POKEPROXY_PORT=tcp://…` into every pod; only env precedence saves pokeproxy | `enableServiceLinks: false` on all three pod specs | Open |
| S3 | Should fix | No runnable deploy procedure and no top-level README (deliverable 8) | A committed deploy doc; superseded by Part 5's entry point | Open |
| S4 | Should fix | `values-prod.yaml` renders rules pointing at the mock Service it disables | `fail` at render time when mock is disabled and a rule has no explicit `url` | Open |
